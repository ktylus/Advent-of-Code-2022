...
def solution(current_position, end, step):
    ...
    if (row - 1, col) not in all_blizzards[step % len(all_blizzards)]:
        ...
        elif row - 1 > 0 and ((row - 1, col, step % len(all_blizzards)) not in states or step < steps_when_entered[(row - 1, col, step % len(all_blizzards))]):
            solution((row - 1, col), end, step)
    if (row, col + 1) not in all_blizzards[step % len(all_blizzards)] and col + 1 < width - 1 and row in range(1, height - 1) and ((row, col + 1, step % len(all_blizzards)) not in states or step < steps_when_entered[(row, col + 1, step % len(all_blizzards))]):
        solution((row, col + 1), end, step)
    if (row + 1, col) not in all_blizzards[step % len(all_blizzards)]:
        if (row + 1, col) == end:
            solution((row + 1, col), end, step)
        elif ((row + 1, col, step % len(all_blizzards)) not in states or step < steps_when_entered[(row + 1, col, step % len(all_blizzards))]) and row + 1 < height - 1:
            solution((row + 1, col), end, step)
    if (row, col - 1) not in all_blizzards[step % len(all_blizzards)] and col - 1 > 0 and row < height - 1 and ((row, col - 1, step % len(all_blizzards)) not in states or step < steps_when_entered[(row, col - 1, step % len(all_blizzards))]):
        solution((row, col - 1), end, step)
    if (row, col) not in all_blizzards[step % len(all_blizzards)] and ((row, col, step % len(all_blizzards)) not in states or step < steps_when_entered[(row, col, step % len(all_blizzards))]):
